...
def create_grid_map(scenario: AoE2DEScenario) -> List[List[int]]:
    ...
    return [[Level.NONE for _ in range(map_size)] for _ in range(map_size)]
...
def mark_blocked_tiles(scenario: AoE2DEScenario, grid_map: List[List[int]]) -> None:
    ...
    for tile in marked_tiles:
        grid_map[tile.y][tile.x] = Level.TERRAIN


def tile_is_valid(tile: Tile, level: Level, grid_map: List[List[int]]) -> bool:
...
def is_blocked(grid_map: List[List[int]], x: int, y: int):
    return grid_map[y][x] != Level.NONE


def is_blocked_for_level(grid_map: List[List[int]], x: int, y: int, level: Level):
    is_temp = grid_map[y][x] == Level.TEMP
